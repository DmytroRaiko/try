...
export const MessageListItem = ({ msg, removeMessage }) => {
  ...
  const { messageId, messageText, senderName, createdAt } = msg

  console.log(Number(msg.userId) === Number(window.localStorage.getItem('user-id')));
  const currentUser = Number(msg.userId) === Number(window.localStorage.getItem('user-id'));
  return (
    <ListGroup.Item
